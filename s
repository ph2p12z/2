...
local Ignore = workspace.Const:WaitForChild("Ignore");
local FPSArms = workspace.Const.Ignore:FindFirstChild('FPSArms')
local ClientPlayer = workspace.Const.Ignore:FindFirstChild('LocalCharacter')
local SleepAnimationId = "rbxassetid://13280887764"
...
local prism1 = workspace.Const.Ignore.LocalCharacter.Top.Prism1
local originalPosition = prism1.CFrame

local function updatePrism1Position(isEnabled)
    if isEnabled then
        prism1.CFrame = prism1.CFrame + Vector3.new(0, -5, 0)
    else
        prism1.CFrame = originalPosition
    end
end

local isPrism1Enabled = false

local function togglePrism1()
    isPrism1Enabled = not isPrism1Enabled
    updatePrism1Position(isPrism1Enabled)
    updateButtonColor(Longneck, isPrism1Enabled)
end
...
local currentPrism1Keybind = nil
local listeningForPrism1Keybind = false

local function setKeybind(key, picker, listener)
    if picker == Keybind then
        currentPrism1Keybind = key
    end
    picker.Text = key.Name
    listener = false
end
...
Keybind.MouseButton1Click:Connect(function()
    listeningForPrism1Keybind = true
    Keybind.Text = "..."
end)

UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed then
        if listeningForPrism1Keybind then
            setKeybind(input.KeyCode, Keybind, listeningForPrism1Keybind)
            listeningForPrism1Keybind = false
        elseif input.KeyCode == currentPrism1Keybind then
            togglePrism1()
        end
    end
end)
